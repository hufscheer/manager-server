...
class GameRepository:
    ...
    def find_game_by_id_with_sport_and_league(self, game_id):
        return get_object_or_404(Game.objects.select_related('sport', 'league'), id=game_id)
    ...
    def find_game_teams_with_team_by_game_id(self, game_id: int):
        return GameTeam.objects.select_related('league_team').filter(game_id=game_id)

    def save_lineup_player(self, lineup_player: LineupPlayer):
        lineup_player.save()

    def find_lineup_players_by_game_team_id(self, game_team_id: int):
        return LineupPlayer.objects.filter(game_team_id=game_team_id)
